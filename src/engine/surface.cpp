...
/// <param name="r">Radius (Diameter / 2)</param>
/// <param name="c">Color of the outline</param>
void Surface::Circle(int cx, int cy, int r, Pixel c)
{
	// Diameter of the circle.
	const int d = (r * 2);

	// Exponentional vars.
	int ex = 1; // X starts slow and gets faster.
	int ey = 1; // Y starts fast and gets slower.

	// The error of the current pixel.
	int error = (ex - d);
	...
	while (x >= y)
	{
		...
		// Move through the Y axis until the error gets too big.
		// (Meaning we're too far from the subpixel position on the x axis)
		if (error <= 0)
		{
			++y;
			error += ey;
		...
		// Do the same as above but for the X axis.
		// (Meaning we're moving through the X axis and each time we reset the Y axis)
		if (error > 0)
		{
			--x;
			ex += 2;
...
void Sprite::Draw( Surface* a_Target, int a_X, int a_Y, bool transparent, bool flip )
{
	...
	if ((x2 > x1) && (y2 > y1))
	{
		...
		for ( int y = 0; y < height; y++ )
		{
			...
			else 
			{
				...
				{
					const Pixel c1 = flip ? *(src + m_Width - x) : *(src + x);
					if (c1 & 0xffffff || !transparent) *(dest + addr + x) = c1;
				}
			}
			addr += dpitch;
			src += m_Pitch;
